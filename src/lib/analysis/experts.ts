...
export function evaluateEnergy(data: IndicatorsResult): ExpertOpinion {
  ...
  if (lastClose > vwap * 1.02) {
    opinion = "상승";
    confidence += 20;
    reasons.push("주가가 거래량 가중평균(VWAP)보다 2% 이상 높아 매수 우위의 수급이 확인됩니다.");
  } else if (lastClose < vwap * 0.98) {
    opinion = "하락";
    confidence += 20;
    reasons.push("주가가 거래량 가중평균(VWAP)보다 낮아 매도 우위 상태입니다.");
  } else {
    reasons.push("주가가 VWAP 부근에서 등락 중이며 팽팽한 수급 공방이 이루어지고 있습니다.");
  }

  // MFI 분석
  if (mfi > 80) {
    opinion = opinion === "상승" ? "하락" : "하락"; // 과매수면 단기 하락위험
    reasons.push("MFI가 80을 넘어 스마트머니의 단계적 차익실현에 주의해야 합니다(과매수 경계).");
    confidence = 70;
  } else if (mfi < 20) {
    opinion = "상승"; 
    reasons.push("MFI가 20 미만으로 바닥권 수급 유입이 기대되는 과매도 구간입니다.");
    confidence = 75;
  } else if (mfi > 50) {
    if (opinion !== "하락") opinion = "상승";
    reasons.push("MFI가 50 이상으로 기본 자금 유입 흐름은 긍정적입니다.");
  } else {
    reasons.push("MFI가 50 미만으로 자금 이탈 현상이 지속 중입니다.");
  }
...
export function evaluateMomentum(data: IndicatorsResult): ExpertOpinion {
  ...
  if (!hasStrongTrend) {
    reasons.push(`ADX가 ${adx.toFixed(1)}로 25 미만이라 확고한 방향성은 부족합니다.`);
  } else {
    reasons.push(`ADX가 ${adx.toFixed(1)}로 강력한 추세가 형성되었습니다.`);
  }
  ...
  if (rsi > 70) {
    opinion = "하락";
    confidence += 20;
    reasons.push("RSI가 70을 상회하며 강력한 단기 고점(과매수) 징후를 나타냅니다.");
  } else if (rsi < 30) {
    opinion = "상승";
    confidence += 20;
    reasons.push("RSI가 30 미만으로 기술적 반등이 절실한 과매도 상태입니다.");
  } else {
    if (pdi > mdi) {
      opinion = "상승";
      confidence += hasStrongTrend ? 25 : 10;
      reasons.push("+DI가 -DI보다 위에 있어 매수 모멘텀이 지배적입니다.");
    } else {
      opinion = "하락";
      confidence += hasStrongTrend ? 25 : 10;
      reasons.push("-DI가 +DI보다 위에 있어 매도 압력이 지배적입니다.");
    }
  }
